...
<head>
    ...
    <script>
      ...
      $('#boton').click(function(){
        ...
        var compruebo = $("#comprobarContra").val();

        if(compruebo != contra){
            $('#resp').html('<p style="color : white;">Contraseñas diferentes!</p>');
        }

        if($.trim(compruebo).length == 0){
          $('#comprobarContra').focus();
          $('#resp').html('<p style="color : white;"> Debes comprobar la contraseña!</p>');
        }

        if($.trim(contra).length == 0){
          $('#contra').focus();
          $('#resp').html('<p style="color : white;"> Contraseña vacia!</p>');
        }
        ...
        if($.trim(id).length > 0 && $.trim(nombre).length > 0 && $.trim(apellido).length > 0 && $.trim(id).length < 9 && $.trim(contra).length > 0  && $.trim(compruebo).length > 0 && compruebo == contra){
        $.ajax({
          url:"../php/ajustando.php",
          method:"POST",
          data: {id:id, nombre:nombre , apellido:apellido , contra:contra },
          cache:"false",
          beforeSend: function(){
...
$('#ajustes').mouseenter(function(){
    ...
    <section id="main-content" class="main">
        <section class="wrapper">
            ...
            <form class="register">
              <div><center><h2 style="color:white;">Datos Personales</h2></center></div>
            <div class="form-group"> <!-- Full Name -->
                <label for="full_name_id" class="control-label">Id</label>
                <input type="text" class="form-control" name="id" placeholder="emp01" maxlengt="8" require disabled="true" value="<?php echo $_SESSION["usuario"]["id"];?>">
            </div>    

            <div class="form-group"> <!-- Street 1 -->
                <label for="street1_id" class="control-label">Nombre</label>
                <input type="text" class="form-control" id="nombre" name="nombre" placeholder="ej. Victor" require value="<?php echo $_SESSION["usuario"]["nombre"];?>">
            </div>                    
                            
            <div class="form-group"> <!-- Street 2 -->
                <label for="street2_id" class="control-label">Apellido</label>
                <input type="text" class="form-control" id="apellido" name="apellido" placeholder="ej. Flores " require value="<?php echo $_SESSION["usuario"]["ape"] ?>">
            </div>    

            <div class="form-group"> <!-- Street 2 -->
                <label for="street2_id" class="control-label">Contraseña</label>
                <input type="password" class="form-control" id="contra" name="apellido" placeholder="ej. Flores " require value="<?php echo $_SESSION["usuario"]["contra"] ?>">
            </div> 

            <div class="form-group"> <!-- Street 2 -->
                <label for="street2_id" class="control-label">Comprobar contraseña</label>
                <input type="password" class="form-control" id="comprobarContra" name="apellido" placeholder="ej. Flores " require value="<?php echo $_SESSION["usuario"]["contra"] ?>">
            </div>
            <input type="hidden" id="id" value="<?php echo $_SESSION["usuario"]["id"];?>"/>
            <div id="resp"></div>
            <div class="form-group"> <!-- Submit Button -->
                <input type="button" value="Modificar datos" class="btn btn-primary" id="boton"/>
            </div>
            </form>                            
        </section>
    </section>
